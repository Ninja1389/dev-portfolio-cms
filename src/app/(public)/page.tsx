...
export default async function HomePage() {
  ...
  const sectionsConfig = (Array.isArray(user?.sectionsConfig)
    ? user!.sectionsConfig
    : [
      ...
      ]) as { id: string; visible: boolean; order?: number }[]

  const sectionMap = new Map(sectionsConfig.map((s) => [s.id, s]))
  ...
  const sections: Record<string, React.ReactNode> = {
    hero: (
      <section key="hero" className="mx-auto max-w-4xl px-6 pt-24 pb-16 md:pt-32 md:pb-24">
        <div className="flex flex-col-reverse md:flex-row items-center gap-8 md:gap-16">
          <div className="flex-1 text-center md:text-left">
            ...
            {user?.title && (
              <p className="mt-4 text-lg text-muted-foreground md:text-xl">{user.title}</p>
            )}
            {user?.bio && (
              <p className="mt-4 max-w-xl text-base leading-relaxed text-muted-foreground">{user.bio}</p>
            )}
            <div className="mt-6 flex items-center justify-center md:justify-start gap-3">
              {socialLinks.map((link, i) => (
                <a
                  ...
                  className="rounded-full p-2.5 text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
                  title={link.platform}
                >
          ...
          {user?.avatarUrl && (
            <div className="shrink-0">
              <img
                src={user.avatarUrl}
                alt={user.name ?? ""}
                className="h-40 w-40 rounded-full object-cover ring-4 ring-muted md:h-56 md:w-56"
              />
            </div>
          )}
        </div>
    ...
    projects: featuredProjects.length > 0 && (
      <section key="projects" className="border-t border-border bg-muted/50">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {featuredProjects.map((project) => (
              ...
              >
                ...
                <h3 className="font-semibold text-foreground group-hover:text-[var(--accent-custom)] transition-colors">
                  {project.title}
                </h3>
                {project.description && (
    ...
    experiences: latestExperiences.length > 0 && (
      <section key="experiences" className="border-t border-border">
        <div className="mx-auto max-w-4xl px-6 py-16 md:py-24">
          <h2 className="text-2xl font-bold tracking-tight text-foreground md:text-3xl mb-10">
            Esperienze recenti
          </h2>
          ...
          <div className="mt-6">
            <Link
              href="/about"
              className="inline-flex items-center gap-1 text-sm font-medium text-[var(--accent-custom)] hover:opacity-80 transition-colors"
            >
    ...
    skills: Object.keys(skillsByCategory).length > 0 && (
      <section key="skills" className="border-t border-border bg-muted/50">
        <div className="mx-auto max-w-4xl px-6 py-16 md:py-24">
          <h2 className="text-2xl font-bold tracking-tight text-foreground md:text-3xl mb-10">
            Competenze
          </h2>
